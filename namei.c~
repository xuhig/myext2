...
struct dentry *myext2_get_parent(struct dentry *child)
{
	...
	unsigned long ino = myext2_inode_by_name(child->d_inode, &dotdot);
	if (!ino)
		return ERR_PTR(-ENOENT);
	return d_obtain_alias(myext2_iget(child->d_inode->i_sb, ino));
} 
	...
	const char * symname)
{
	...
	if (l > sizeof (MYEXT2_I(inode)->i_data)) {
		...
		memcpy((char*)(MYEXT2_I(inode)->i_data),symname,l);
		inode->i_size = l-1;
	}
	mark_inode_dirty(inode);
